    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result.college, function (idx, elem) {
                ...
                table.append("<tr  class='odd gradeX'><td >" + elem.name + "</td><td style='display: none;' >" + elem.description + "</a></td>" + "<td style='display: none;'> " + elem._id + "</td><td><button class='btn btn-primary' onclick='editModal(this)'  data-toggle=\"modal\" data-target=\"#modalEdit\">ویرایش</button> </td><td><button class='btn btn-dark' onclick='editModal(this)'  data-toggle=\"modal\" data-target=\"#modalVillage\">اضافه کردن رشته</button> </td><td><button onclick='addRowHandlers(this)' class='btn btn-danger' >مشاهده</button></td></tr>");
...
function saveFaculty() {
    ...
    let object = {};
    object.name = $('#nameVillage').val();
    object.description = $('#descriptionVillage').val();
    object.collegeId = $('#id').val()
    form.append('name' , object.name)  ;
    form.append('description' , object.description)
    var input = $('#upload-photo_2')[0]
    const file = input.files[0]
    form.append('logo', file)
    form.append('collegeId' , object.collegeId )
