...
function ajaxGet(page) {
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result.document, function (idx, elem) {
                ...
                if (elem.authors[0]) {
                    if (elem.active == true) {
                        table.append("<tr  class='odd gradeX'><td> " + elem.name + "</td><td  >" + m.format('YYYY/M/D') + "</td><td  >" + elem.authors[0].fullName + "</td><td><button onclick='changeActive(this)'   class='btn btn-success disabled'> " + "فعال" + "</button></td>" + "<td style='display: none;'> " + elem._id + "</td>" + "<td><button onclick='editBlog()'   class='btn btn-dark'> " + "ویرایش" + "<i class='fa fa-pencil'></i></button></td><td><button type='button' class='btn btn-secondary' onclick='addRowHandlers()'>دانلود</button> </td>" + "</tr>");
                    } else {
                        table.append("<tr class='odd gradeX'><td >" + elem.name + "</td><td  >" + m.format('YYYY/M/D') + "</td><td >" + elem.authors[0].fullName + "</td><td><button onclick='changeActive(this)'   class='btn btn-outline-danger active'> " + "غیرفعال" + "</button></td>" + "<td style='display: none;'> " + elem._id + "</td>" + "<td><button onclick='editBlog()'   class='btn btn-dark'> " + "ویرایش" + "<i class='fa fa-pencil'></i></button></td><td><button type='button' class='btn btn-secondary' onclick='addRowHandlers()'>دانلود</button> </td>" + "</tr>");
    ...
    for (i = 0; i < rows.length; i++) {
        ...
        var createClickHandler = function (row) {
            return function () {
                ...
                // location.href = `/upload/doc/` + id
                $.ajax({
                    type: "GET",
                    url: "/doc/" + id,
                    success: function (result) {
                        location.href = `/upload/doc/` + result.file
...
function blogcategoty() {
    $.ajax({
        ...
        success: function (result) {
            ...
            let college = $('#collegeSelect')

            college.append('<option value=' + '' + ">" + "انتخاب دانشکده" + "</option>")


            $.each(result.college, function (idx, elem) {
                blogCategory.append('<option value=' + `${elem._id}` + ">" + elem.name + "</option>")
                college.append('<option value=' + `${elem._id}` + ">" + elem.name + "</option>")
...
function saveDoc() {
    let collegeId = $('#collegeSelect option:selected').val()
    let facultyId = $('#facultySelect option:selected').val()
...
function fillFaculty() {
    ...
    $.ajax({
        type: "GET",
        url: "/college/faculty/all/" + collegeId +"/1-100",
        success: function (result) {
            console.log(result)
            let faculty = $('#facultySelect')



            $.each(result.faculty, function (idx, elem) {
                faculty.append('<option value=' + `${elem._id}` + ">" + elem.name + "</option>")
