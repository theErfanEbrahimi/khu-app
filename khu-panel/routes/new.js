...
var eventService = require('../services/news')
...
router.get('/:newId', utils.authentication, utils.accessControl('admin', 'r', '-'), (req, res, next) => {
    eventService.getPost(req.params.newId)
        .then((post) => {
            res.status(200).send(post)
        })
        .catch(err => {
...
router.post('/verification' , utils.authentication , utils.accessControl('admin' , 'r' , 'w') , function (req , res , next) {
    eventService.activeNew(req.body.id , req.body.active)
        .then(() => {
